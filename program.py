...
print('\t\t\tHi\n \tThis is Addy, Your Personal System Assistant')
pyttsx3.speak("Hi, This is Addy, your personal system assistant, your own application guide. Let me help you with the applications on this computer")
while True:
    print('\t----------------------------------------------------------------------------------')
    print('\t\tCHAT WITH ME: ', end='')
    pyttsx3.speak('please chat with me')
    p=input()
    if (('run' in p) or ('execute' in p) or ('open' in p) or ('launch' in p) or ('send' in p) or ('show' in p)):
        if ('email' in p) or ('email' in p) or ('Email' in p) or ('Gmail' in p) or ('mail' in p):
            ...
            if your_choice=="y":
                ...
                pyttsx3.speak('enter sender mail passcode')
        ...
        pyttsx3.speak("Hello Folk!! Please let me know which application you want to open.")

    elif ('exit' in p) or ('close' in p) or ('terminate' in p) or ('stop' in p):
